...
## Общее
Монорепозиторий на pnpm (без workspaces-манифеста), с кастомным раннером сборки `tools/build.js`.
Node >= 20, пакетный менеджер — pnpm.
...
- `npm/` — результат сборки (генерируется командой `pnpm build`, в `.gitignore`).
...
- `pnpm install` — установка зависимостей (проходит, есть несколько предупреждений о peer deps,
  не блокирующих).
- `pnpm build` → `node tools/build` — собирает все 5 таргетов (`effector`, `effector-react`,
  `effector-vue`, `effector-solid`, `forest`) примерно за 20 секунд, кладёт результат в `npm/`.
- `pnpm test` — запуск тестов через Vitest, есть отдельные конфигурации для
