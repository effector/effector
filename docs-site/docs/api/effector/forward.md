...
core team recommends [sample](docs/api/effector/sample.md) instead
:::
...
1. `from` ([Unit | Unit\[\]](docs/explanationglossary.md#common-unit)): Source of updates. Forward will listen for changes of these units
   ...
   - if an array of [units](docs/explanationglossary.md#common-unit) is passed, `to` will be triggered when any unit in `from` array is triggered

2. `to` ([Unit | Unit\[\]](docs/explanationglossary.md#common-unit)): Target for updates. `forward` will trigger these units with data from `from`
   ...
   - if `to` is an array of [units](docs/explanationglossary.md#common-unit), each unit in that array will be triggered

**Returns**

[Subscription](docs/explanationglossary.md#subscription): Unsubscribe function. It breaks connection between `from` and `to`. After call, `to` will not be triggered anymore.
...
[_effect_]: docs/api/effector/Effect.md
[_store_]: docs/api/effector/Store.md
[_event_]: docs/api/effector/Event.md
