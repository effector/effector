...
Accepts an array of module names which exports treat as custom factories therefore each function call provides unique prefix for [sids](docs/api/effector/babel-plugin.md#sid) of units inside them. Used to SSR([Server Side Rendering](docs/api/effector/Scope.md)) and it's not required for client-only application.
...
Import `createEffectStatus` from `'./createEffectStatus'` was treated as factory function so each store created by it has its own [sid](docs/api/effector/babel-plugin.md#sid) and will be handled by [serialize](docs/api/effector/serialize.md) independently, although without `factories` they will share the same `sid`.
