...
Mode in which target case is selected by name of it's field. Case could be selected from data in `source` by [case function](docs/ru/api/effector/split.md#case-function) or from external [case store](docs/ru/api/effector/split.md#case-store) which keept current case name. After selection data from `source` will be sent to corresponding `cases[fieldName]` (if there is one), if none of the fields matches, then the data will be sent to `cases.__` (if there is one)

**See also**:

- [case store](docs/ru/api/effector/split.md#case-store)
- [case function](docs/ru/api/effector/split.md#case-function)
...
- [matching store](docs/ru/api/effector/split.md#matcher-store)
- [matching function](docs/ru/api/effector/split.md#matcher-function)
...
String-returning function which will be called with value from `source` to choose case by it's name. Placed directly in `match` field, [should be **pure**](docs/ru/glossary.mdmd#purity)
...
Boolean store which indicates whether to choose particular case or try next one. Placed in fields of `match` object, might be mixed with [matcher functions](docs/ru/api/effector/split.md#matcher-function)
...
Boolean-returning function which indicates whether to choose particular case or try next one. Placed in fields of `match` object, might be mixed with [matcher stores](docs/ru/api/effector/split.md#matcher-store), [should be **pure**](docs/ru/glossary.mdmd#purity)
...
- `source`: [Unit](docs/ru/glossary.mdmd#common-unit) which will trigger computation in `split`
- `match`: Single [store with string](docs/ru/api/effector/split.md#case-store), single [function which returns string](docs/ru/api/effector/split.md#case-function) or object with [boolean stores](docs/ru/api/effector/split.md#matching-store) and [functions which returns boolean](docs/ru/api/effector/split.md#matching-function)
- `cases`: Object with [units](docs/ru/glossary.mdmd#common-unit) to which data will be passed from `source` after case selection
...
1. `source`: [Unit](docs/ru/glossary.mdmd#common-unit) which will trigger computation in `split`
...
It works the same like [split with cases](docs/ru/api/effector/split.md#split-with-cases), however computations in `split` will be started after `clock` is triggered.
