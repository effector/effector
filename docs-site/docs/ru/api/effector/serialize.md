...
A companion method for [_fork_](docs/ru/api/effector/fork.md). Allows to get a serialized value for all the store states within a [scope](docs/ru/api/effector/Scope.md). The main purpose is an application state serialization on the server side during SSR.

**Arguments**

1. `scope` [_Scope_](docs/ru/api/effector/Scope.md): a scope object (forked instance)
2. `ignore` Optional array of [_Store_](docs/ru/api/effector/Store.md) to be omitted during serialization (added 20.14.0)
...
::: warning
If a store [does not have a sid](docs/ru/api/effector/babel-plugin.md#sid), its value will be omitted during serialization.
:::
...
With `onlyChanges`, this method will serialize only stores which were changed by some trigger during work or defined in `values` field by [fork](docs/ru/api/effector/fork.md) or [hydrate(scope)](docs/ru/api/effector/hydrate.md). Once being changed, a store will stay marked as changed in given scope even if it was turned back to default state during work, otherwise client will not update that store on its side, which is unexpected and inconsistent.
