...
import {createReduxStore as createStore} from '..'
...
describe('createStore', () => {
  ...
  describe('supports multiple subscriptions', () => {
    it('supports multiple subscriptions (original)', () => {
      const store = createStore(reducers.todos)
      ...
      store.dispatch(unknownAction())
      /**
       * NOTE: effector will not make useless updates,
       * thus there will be zero calls
       * instead of one in redux
       */
      expect(listenerA.mock.calls.length).not.toBe(1)
       ...
       * NOTE: effector will not make useless updates,
       * thus there will be zero calls
       * instead of two in redux
       */
      expect(listenerA.mock.calls.length).not.toBe(2)
      expect(listenerA.mock.calls.length).toBe(0)
      expect(listenerB.mock.calls.length).toBe(0)
       ...
       * NOTE: effector will not make useless updates
       */
      expect(listenerA.mock.calls.length).not.toBe(3)
      expect(listenerB.mock.calls.length).not.toBe(1)
      expect(listenerA.mock.calls.length).toBe(0)
      expect(listenerB.mock.calls.length).toBe(0)
       ...
       */
      expect(listenerA.mock.calls.length).not.toBe(3)
      expect(listenerB.mock.calls.length).not.toBe(2)
      expect(listenerA.mock.calls.length).toBe(0)
      expect(listenerB.mock.calls.length).toBe(0)
       ...
       * NOTE: effector will not make useless updates
       */
      expect(listenerA.mock.calls.length).not.toBe(4)
      expect(listenerB.mock.calls.length).not.toBe(2)
      expect(listenerA.mock.calls.length).toBe(0)
      expect(listenerB.mock.calls.length).toBe(0)
    })

    it('supports multiple subscriptions (real)', () => {
      const store = createStore(reducers.todos)
      const listenerA = jest.fn()
      const listenerB = jest.fn()
  ...
  it.skip('only accepts plain object actions', () => {
    const store = createStore(reducers.todos)
    expect(() => store.dispatch(addTodo(''))).not.toThrow()
