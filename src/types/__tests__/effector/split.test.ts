...
test('split + attach in default case (should pass)', () => {
...
test('split + attach in default case (should fail)', () => {
  const $number = createStore<number>(0)
  split({
    //@ts-expect-error
    source: $number,
...
test('split + inline prepend call (should pass)', () => {
  const fooFx = createEffect(() => 'foo')
