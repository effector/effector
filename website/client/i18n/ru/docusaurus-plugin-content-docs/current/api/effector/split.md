...
Стор со строкой, которая будет использоваться для выбора случая по его имени. Размещается непосредственно в поле `match`
...
Функция возвращающая строку, которая будет вызвана со значением из `source` для выбора случая по его имени. Размещается непосредственно в поле `match`, [должно быть **чистым**](../../glossary.md#purity)
...
Функция возвращающая `boolean`, указывающая, следует ли выбрать конкретный случай или попробовать следующий. Размещается в полях объекта `match`, может смешиваться с [matcher stores](./split.md#matcher-store), [должны быть **чистыми**](../../glossary.md#purity)
