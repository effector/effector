...
type OptionalParams<Args extends any[]> = Args['length'] extends 0 // does handler accept 0 arguments?
  ...
  ? /**
    ...
    Args[0] | void
  : Args[0]
type EffectByHandler<FN extends Function, Fail> = FN extends (
  ...args: infer Args
) => infer Done
...
export interface Event<Payload> extends Unit<Payload> {
  ...
  (
    this: IfUnknown<
      Payload,
      void,
      Payload extends void ? void : `Error: Expected 1 argument, but got 0`
    >,
    payload?: Payload,
  ): void
...
export interface Store<State> extends Unit<State> {
  ...
  on<E extends Tuple<Unit<any>>>(
    ...
    reducer: (
      state: State,
      payload: InferValueFromTupleOfUnits<E>,
    ) => State | void,
  ): this
...
export class Domain implements Unit<any> {
  ...
  createEffect<FN extends Function, Fail>(
    handler: FN,
...
export const step: {
  ...
  run(data: {
    fn: (data: any, scope: {[field: string]: any}, stack: Stack) => any
  }): Compute
  mov(data: {
 ...
 */
export function createEffect<FN extends Function>(
  handler: FN,
): EffectByHandler<FN, Error>
/**
 * Creates an effect
 ...
 * @param handler function to handle effect calls
 */
export function createEffect<FN extends Function, Fail>(
  handler: FN,
): EffectByHandler<FN, Fail>
/**
 * Creates an effect
...
> = UnitValue<Source> extends CaseTypeReader<Cases, keyof Cases>
  ? Match extends Unit<any>
    ...
    : Match extends Record<
        string,
        ((p: UnitValue<Source>) => boolean) | Store<boolean>
      >
        ...
        ? Cases extends CaseRecord
          ? Clock extends Unit<any> | Array<Unit<any>>
            ? SplitType<
                Cases,
                Match,
                {source: Source; match: Match; cases: Cases; clock: Clock},
                Source
              >
            : {error: 'clock should be a unit or array of units'; got: Clock}
          : {
              error: 'cases should be an object with units or arrays of units'
              got: Cases
            }
      ...
      ? Source extends Unit<any>
        ? Cases extends CaseRecord
          ? SplitType<
              Cases,
              Match,
              {source: Source; match: Match; cases: Cases},
              Source
            >
          : {
              error: 'cases should be an object with units or arrays of units'
        ...
        : {error: 'source should be a unit'; got: Source}
      : {
          error: 'config should be object with fields "source", "match" and "cases"'
          got: Config
        }
...
type MatcherInferenceValidator<Cases, Match> = {
  [K in keyof Match as Match[K] extends (p: any) => p is infer R
...
type CaseTypeReader<Cases, K extends keyof Cases> = Cases[K] extends infer S
  ? WhichType<UnitValue<S extends Array<any> ? S[number] : S>> extends 'void'
    ? unknown
    : UnitValue<S extends Array<any> ? S[number] : S>
  : never

type CaseValue<Cases, Match, K extends keyof Match> = K extends keyof Cases
...
type IfAssignable<T, U, Y, N> = (<G>() => IfAny<
  T & U,
  0,
  G extends T ? 1 : 2
>) extends <G>() => IfAny<T & U, 0, G extends U ? 1 : 2>
  ? Y
  ...
  ? number extends T['length']
    ...
    ? number extends U['length']
      ? N
      : TupleObject<T> extends TupleObject<U>
      ? Y
      : N
    : N
  : N
...
type GetMergedValue<T> = GetTupleWithoutAny<T> extends never
  ? any
  : GetTupleWithoutAny<T>
 ...
 */
type ReplaceUnit<Target, Result, Value> = IfAssignable<
  Result,
  Value,
  Target,
  Value extends void ? Target : 'incompatible unit in target'
>
...
type SampleImpl<
  ...
  FN,
  FNInf,
  FNInfSource extends Source extends Unit<any> | SourceRecord
    ? TypeOfSource<Source>
    : never,
  ...
  unknown extends Target
    ? // no target, no source
      ...
      ? unknown extends Clock
        ...
        : [
            message: {
              error: 'clock should be unit or array of units'
              got: Clock
            },
          ]
...
type ModeSelector<FilterAndFN, FilterOnly, FNOnly, None, SomeFN> =
  ...
  ? unknown extends Clock
    ...
    ? Source extends Unit<any> | SourceRecord
      ? // has filter, has fn
        ...
        ? FLUnit extends Unit<any>
          ? FN extends (
              src: TypeOfSource<Source>,
              clk: TypeOfClock<Clock>,
            ) => any
            ? EventAsReturnType<ReturnType<FN>>
            : never
          : FLBool extends BooleanConstructor
          ? FNAltArg extends (
              arg: NonFalsy<TypeOfSource<Source>>,
              clk: TypeOfClock<Clock>,
            ) => any
            ? EventAsReturnType<ReturnType<FNAltArg>>
            : never
...
export function sample<
  ...
  FNInfSource extends Source extends Unit<any> | SourceRecord
    ...
    : never,
  FNInfClock extends Clock extends Units ? TypeOfClock<Clock> : never,
  FilterFun extends Source extends Unit<any> | SourceRecord
    ? Clock extends Units
      ?
          | never
          | ((
    ...
    : never,
  FN extends Source extends Unit<any> | SourceRecord
    ? Clock extends Units
    ...
    : never,
  FNInf extends Source extends Unit<any> | SourceRecord
    ? Clock extends Units
    ...
    ? (clk: FNInfClock) => any
    : never,
  FNNonFalsy extends Source extends Unit<any> | SourceRecord
    ? NonFalsy<TypeOfSource<Source>>
    : Clock extends Units
    ? NonFalsy<TypeOfClock<Clock>>
    : never,
  FNAltArg extends Source extends Unit<any> | SourceRecord
    ? Clock extends Units
  ...
  SourceNoConf,
  ClockNoConf,
  FNSrcNoConf extends SourceNoConf extends Unit<any> | SourceRecord
    ? (src: TypeOfSource<SourceNoConf>) => any
    : never,
  FNBothNoConf extends SourceNoConf extends Unit<any> | SourceRecord
  ...
  InferTarget,
>(
  ...args: SourceNoConf extends Unit<any> | SourceRecord
    ? ClockNoConf extends Units
...
type TargetConfigCheck<
  ...
  Config,
  SomeFN,
> = Mode extends Mode_Fn_Trg // mode with fn
  ? // there should be an explicit conditional selection
    ...
    ? TargetOrError<ReturnType<FN>, 'fnRet', Target, Config & SomeFN>
  ...
  ? Source extends Unit<any> | SourceRecord
    ? TargetOrError<TypeOfSource<Source>, 'src', Target, Config & SomeFN>
    : [
        message: {
          error: 'source should be unit or object with stores'
          got: Source
  ...
  ? Clock extends Units
    ? TargetOrError<TypeOfClock<Clock>, 'clk', Target, Config & SomeFN>
    : [message: {error: 'clock should be unit or array of units'; got: Clock}]
  : never

type InferredType<Source, Clock, FilterFN> = Source extends
  | Unit<any>
  | SourceRecord
  ? Clock extends Units
  ...
  ? FLUnit extends Unit<any>
    ? boolean extends UnitValue<FLUnit>
      ? FN extends DataSourceFunction<Source, Clock>
        ...
        : [
            message: {
              error: 'filter unit should has boolean type'
              got: UnitValue<FLUnit>
            },
          ]
      : [message: {error: 'function should accept data source types'; got: FN}]
    : FLBool extends BooleanConstructor
    ? Mode extends Mode_Flt_Fn_Trg
      ...
      ? Source extends Unit<any> | SourceRecord
        ? TargetOrError<
            ...
            'src',
            Target,
            TargetFilterFnConfig<Mode, Target, Source, Clock, FLBool, never> &
              SomeFN
          >
        : [
            message: {
    ...
    ? // mode with fn
      ...
      ? FNInf extends (
        ...
        : [
            message: {
              error: 'function should accept data source types'
              got: FNInf
            },
          ]
      : // mode with source only or with both clock and source
    ...
    ? ReturnType<FilterFun> extends boolean
      ? // mode with fn
        ...
        ? FN extends DataSourceFunction<Source, Clock>
          ? TargetOrError<
              ...
              'fnRet',
              Target,
              TargetFilterFnConfig<Mode, Target, Source, Clock, FilterFun, FN> &
                SomeFN
            >
          : [
              message: {
      ...
      : [
          message: {
            error: 'filter function should return boolean'
            got: ReturnType<FilterFun>
          },
        ]
...
> = Mode extends Mode_Flt
  ? FLUnit extends Unit<any>
    ? boolean extends UnitValue<FLUnit>
      ? [
          config: (Mode extends 'clock | source | filter | fn |       '
            ? {
                clock: Clock
                source: Source
                filter?: FLUnit
    ...
    ? Mode extends Mode_Flt_Fn
      ...
      : [
          config: (Mode extends 'clock | source | filter |    |       '
            ? {
                clock: Clock
                source: Source
                filter: FLBool
      ...
      ? FNInf extends (
        ...
        : [
            message: {
              ...
              inferred: Source extends Unit<any> | SourceRecord
                ? FNInfSource
                : FNInfClock
  ...
  : Mode extends Mode_NoFlt
  ? [
      config: (Mode extends 'clock | source |        | fn |       '
        ? {
            clock: Clock
            source: Source
            filter?: never
...
type DataSourceFunction<Source, Clock> = Source extends Unit<any> | SourceRecord
  ? Clock extends Units
    ? (src: TypeOfSource<Source>, clk: TypeOfClock<Clock>) => any
    : (src: TypeOfSource<Source>) => any
  : Clock extends Units
...
> = Target extends Unit<any>
  ? Target extends Unit<infer TargetType>
    ...
    : never
  : {
      [K in keyof Target]: Target[K] extends Unit<infer TargetType>
        ? [SourceType] extends [Readonly<TargetType>]
          ? Target[K]
          : WhichType<TargetType> extends 'void' | 'any'
          ? Target[K]
  ...
  Clock extends never[]
    ...
    ? WhichType<
        UnitValue<
          ClockValueOf<
            {
              [K in keyof Clock as WhichType<UnitValue<Clock[K]>> extends 'any'
                ? never
                : K]: Clock[K]
            }
          >
...
type GuardFilterSC<S, C> =
  ...
  | Store<boolean>
type GuardFilterS<S> = ((source: GetSource<S>) => boolean) | Store<boolean>
type GuardFilterC<C> = ((clock: GetClock<C>) => boolean) | Store<boolean>
...
>(config: {
  ...
  target: F extends (value: any, ...args: any) => value is infer X
    ...
    : MultiTarget<
        T,
        IfUnknown<UnitValue<S>, GetGuardClock<C, F>, GetGuardSource<S, F>>
      >
  name?: string
  greedy?: boolean
}): T
...
export function allSettled<FX extends Effect<any, any, any>>(
  ...
  | {status: 'done'; value: EffectResult<FX>}
  | {status: 'fail'; value: EffectError<FX>}
>
/**
