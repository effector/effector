...
test('smoke', async() => {
 ...
 const store: Store<{foo: 'bar'}> = createEng(
  (s = {foo: 'bar'}, x) => (fn(x), console.log(x), s),
  void 0,
...
test('hot reload support', async() => {
 ...
 const used = jest.fn(x => Promise.resolve(x))
 const usedDone = jest.fn(x => Promise.resolve(x))
 const storeA: Store<{foo: 'bar'}> = createEng(
  (s = {foo: 'bar'}, x) => (fnA(x), console.log(x), s),
  void 0,
  applyMiddleware(effectorMiddleware),
 )
 const domain = createRootDomain()
 ...
 const storeB: Store<{foo: 'bar'}> = createEng(
  (s = {foo: 'bar'}, x) => (fnB(x), console.log(x), s),
  void 0,
  applyMiddleware(effectorMiddleware),
 )
