...
import {Atomic} from '@effector/atom'
...
export type Effect<Params, Done, Fail, State> = {
 ...
 // port<R>(events$: Stream<R>): Promise<void>,
 use(thunk: (params: Params) => Promise<Done> /*:: | Done*/): void,
 trigger(
...
export type Reducer<S> = {
 ...
 lastState: Atomic<S>,
 get(): S,
 set(value: S): Reducer<S>,
 watch(fn: (S) => any): () => void,
 map<B>(fn: (S) => B): Reducer<B>,
 options(opts: {fallback: boolean}): Reducer<S>,
 //  has(event: Event<any, any>): boolean,
 on<P, A /*: Event<P, any> | $ReadOnlyArray<Event<any, any>>*/>(
  event: A,
  handler: (state: S, payload: P, meta: Meta) => S,
 ): Reducer<S>,
 off<A /*: Event<any, any> | $ReadOnlyArray<Event<any, any>>*/>(
  event: A,
 ): Reducer<S>,
 reset<A /*: Event<any, any> | $ReadOnlyArray<Event<any, any>>*/>(
  event: A,
 ): Reducer<S>,
 subscribe(subscriber: Subscriber<S>): Subscription<S>,
}

export type Handler<S, P> = (state: S, payload: P, meta: Meta) => S
