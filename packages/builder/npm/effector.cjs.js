...
function createWatcher({
  ...
  return function watch(fn) {
    event.epic((data$, state$) => most.combine((data, state) => ({
      data,
      state
...
function EventConstructor(domainName, dispatch, getState, state$, events, name, action$ = mostSubject.sync(), config = {
  isPlain: false,
  watchFailCheck: 'warn'
}) {
...
function EffectConstructor(domainName, dispatch, getState, state$, events, name, opts, action$ = mostSubject.sync()) {
  const handlers = new Set();
...
function createDomain(store, domainName = '') {
  ...
  const state$ = most.from(redux).map(() => store.getState()).multicast();
...
function createRootDomain(domainName = '') {
  ...
  function register(store) {
    ...
    const unsub = most.from(store).map(() => getState()).subscribe({
      next(value) {
        state$.next(value);
      },
...
//@noflow
function combine(combine, ...reducers) {
  const values = reducers.map(reducer => reducer(undefined, {
    type: '@@effector/void'
  }));
  const defaultValues = [...values];
  const defaultState = combine(...values);
...
exports.createRootDomain = createRootDomain;
exports.effectorMiddleware = effectorMiddleware;
exports.createReducer = createReducer;
exports.combine = combine;
exports.joint = combine;
exports.createHaltAction = createHaltAction;
exports.createEvent = createEvent$1;
exports.createEffect = createEffect;
exports.mill = collect;
