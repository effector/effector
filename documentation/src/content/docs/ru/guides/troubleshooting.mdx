...
Эта ошибка сообщает вам о том, что вы пытаетесь передать в ваш store значение `undefined`, что, возможно, является некорректным поведением.

Если вам действительно нужно передать ваш store значение `undefined`, то вам надо вторым аргументом в `createStore` передать объект со свойством `skipVoid: false`.
...
Эта ошибка часто встречается при работе с SSR, она связана с тем, что у вашего store отсутствует `sid` (stable id), который необходим для корректной гидрации данных с сервера на клиент.
