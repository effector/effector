...
Работа юнитов в effector всегда происходит внутри скоупа — глобального или изолированного, созданного через [`fork()`](/ru/api/effector/fork). В глобальном случае контекст потерять невозможно, так как он используется по умолчанию. С изолированным скоупом всё сложнее: при потере скоупа операции начнут выполняться в глобальном режиме, а изменения данных не попадут в нужный скоуп, из-за чего клиент получит неконсистентное состояние.
...
Заметьте, что метод [`scopeBind`](/ru/api/effector/scopeBind) сам умеет работать с текущим используемым скопом. Однако, если вам нужно, то вы можете передать нужный скоуп в конфигурационный объект вторым аргументом.
