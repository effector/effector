...
Для обработки изоморфного SSR с effector нам нужен надежный способ [`сериализации`](/ru/api/effector/serialize/) состояния, чтобы передать его по сети. Для этого нам нужно иметь стабильные идентификаторы (sid'ы) для каждого store в нашем приложении.
...
2. Изоморфный SSR требует небольшой специальной подготовки – вам понадобятся [sid для store'ов](/ru/explanation/sids).
