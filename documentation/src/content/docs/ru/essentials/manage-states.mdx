...
Данные store'ов в effector иммутабельные – это значит, что вы не должны мутировать массивы или объекты напрямую, а создавать новые инстансы при обновлениях.
:::
...
## Создание store (#store-creation)

Создать новый store можно при помощи метода [`createStore`](/ru/api/effector/createStore):

```ts
import { createStore } from "effector";

// Создание store с начальным значением
...
:::tip{title="Наименование store'ов"}
В effector принято использовать префикс `$` для store'ов. Это улучшает ориентацию в коде и опыт поиска в вашей IDE.
:::

## Чтение значений (#read-store-value)

Получить текущее значение store можно несколькими способами:
...
Состояние store обновляется когда получает значение, которое не равно (!==) текущему, а также не равно `undefined`.
:::

### Обновление через события (#update-store-via-events)

Самый простой и верный способ обновить store - это привязать его к событию:
...
Обновить store можно и с помощью параметров события, достаточно лишь передать данные в событие, как у обычной функции, и использовать в обработчике:
...
При помощи метода `on` мы можем обновить состояние store для простых случаев, при срабатывании какого-то события, передав данные из события либо обновить на основе предыдущего значения.
...
- Нужно контролировать обновление store при помощи события
- Требуется обновить store на основе значений других store'ов
- Нужна трансформация данных перед обновлением store с доступом к актуальным значениям других store'ов
...
// всех связанных store'ов в момент срабатывания события
sample({
  clock: updateItems,
...
1. Доступ к актуальным значениям всех store'ов
2. Атомарность обновлений нескольких store'ов
...
### Создание store при помощи метода `restore` (#create-store-via-restore)
...
Также метод `restore` можно использовать и с эффектом, в таком случае в store попадут данные из события эффекта [`doneData`](/ru/api/effector/Effect#doneData), а дефолтное значение store должно соответствовать возвращаемому значению:
...
Когда вам нужно создать множество обработчиков для одного store, вместо создания отдельных событий и подписки на них, вы можете использовать [`createApi`](/ru/api/effector/createApi). Эта функция создает набор событий для обновления store в одном месте.<br/>
...
Часто нужно создать store, значение которого зависит от других состояний. Для этого используется метод [`map`](/ru/api/effector/Store#map):
...
// Комбинация нескольких store'ов
const $friendsList = combine($users, $currentUser, (users, currentUser) =>
  users.filter((user) => user.friendIds.includes(currentUser.id)),
);
```

Мы также использовали здесь метод [`combine`](/ru/api/effector/combine), который позволяет нам объединить значение нескольких store'ов в одно.<br/>
...
Производные сторы обновляются автоматически при изменении исходных store'ов. **Не нужно** вручную синхронизировать их значения.
:::

## Сброс состояния (#resetting-store-state)

Вы можете сбросить состояние store до исходного при помощи метода `reset`:
...
По умолчанию effector пропускает обновления со значением `undefined`. Это сделано для того, чтобы можно было ничего не возвращать из редьюсеров, если обновление store не требуется:
...
Как показала практика, будет лучше просто возвращать предыдущее значение store.
:::

Если вам нужно использовать `undefined` как валидное значение, необходимо явно указать с помощью `skipVoid: false` при создании store:
...
[Познакомиться с полным API для store'ов тут](/ru/api/effector/Store)
