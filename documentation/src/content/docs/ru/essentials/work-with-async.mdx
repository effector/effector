...
Код выше абсолютно вылидный и отработает как надо, однако если мы перехватим ошибку с помощью события [`failData`](/ru/api/effector/Effect#properties-failData), то заметим тип `Error` вместо нашей кастомной ошибки `CustomError`:
