...
## Типизация store'ов (#typing-stores)
...
// Базовый store с примитивным значением
...
### Типы store'ов (#store-types)

В эффектор существуют два типа store'ов, где `T` - тип хранимого значения:

1. `Store<T>` - тип производного store, в который нельзя записать новые данные.
2. `StoreWritable<T>` - тип store, в который можно записывать новые данные при помощи `on` или `sample`.
