...
- упомянуть про статику обязательно и добавить ссылку на эту страницу
- вызов события осуществляется либо декларативно либо императивно (sample, onClick и тд тп)
...
:::info{title="TODO"}
События effector не являются заменой событиям DOM, данный пример служит демонстрацией похожей концепции работы.
:::
...
Императивным способ вызова это наиболее привычный, когда мы вызываем событие как функцию `event()`, такой способ, скорее всего, вы будете использовать при вызове события в UI, когда передаете в `onClick` обработчик:
...
Декларативный вызов это, например, когда мы подписываемся событием на другое событие с помощью метода `sample` и аргумента `target`. Событие переданное в `target` у метода `sample` вызовется при срабатывании `clock`:
...
{/* TODO добавить ссылки на createAction после мержа */}
:::info{title="А можно ли вызывать императивно в sample?"}
