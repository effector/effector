...
1. Безопасность типов для состояний, store'ов и событий
...
// Когда store `$session` обновляется, то store `$isLogged` тоже будет обновлен
// Они синхронизированы. Производный store зависит от данных из исходного
...
Это связано с тем, что мы создали store `$loggedIn` в модели, но не изменяем его. Давайте исправим:
...
Здесь мы просто реэкспортировали наш собственный store из сущности сессии, но слой представления не меняется.
...
Это связано с тем, что store `$session` содержит `Session | null`, а `messageSendFx` хочет `Author` в аргументах.
...
потому что store `$messages` не обновился. Мы можем написать универсальный код для этого случая.
