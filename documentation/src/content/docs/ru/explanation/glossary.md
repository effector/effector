...
## Производный стор (Derived store) (#derived-store)

_Производный стор_ (_derived store_) это стор только для чтения, который создается из других сторов и автоматически обновляется при изменении исходных сторов. Производные сторы создаются с помощью методов таких как [Store.map](/ru/api/effector/Store#methods-map-fn), [combine](/ru/api/effector/combine), или как свойства эффектов (например, [Effect.pending](/ru/api/effector/Effect#properties-pending)).

Ключевые особенности:
- Не может быть изменен напрямую событиями
- Не может быть использован как `target` в [sample](/ru/api/effector/sample)
