...
К счастью, существуют [`effector/babel-plugin`](/api/effector/babel-plugin) и [`@effector/swc-plugin`](/api/effector/swc-plugin), которые автоматически создадут sid.
...
:::tip
Предпочтительно использовать [`effector/babel-plugin`](/api/effector/babel-plugin) или [`@effector/swc-plugin`](/api/effector/swc-plugin) вместо добавления sid вручную.
:::
...
Здесь нам очень помогут sid'ы. Поскольку sid, по определению, одинаков для одного и того же store в любом окружении, effector может просто полагаться на него для обработки сериализации состояния и гидрации.
...
Благодаря sid'ам, гидрация состояния также работает автоматически:
...
Но этого недостаточно, потому что мы можем создать два экземпляра `createName`, и внутренние stores обоих этих экземпляров будут иметь одинаковые sid!
