...
### Создаем ваш первый store (#first-store)

Теперь давайте создадим store, который является состоянием вашего приложения.
...
Дальше давайте добавим события, при вызове которых, мы будем обновлять данные store.
...
### Подписываем store на событие (#connection-event-to-store)
