...
lang: ru
description: Основные концепции эффектора - store, эффект, событие и как все это работает вместе
redirectFrom:
...
В основе Effector лежит концепция **юнитов** - независимых строительных блоков приложения. Каждый юнит: [store (store)](/ru/essentials/manage-states), [событие (event)](/ru/essentials/events) или [эффект (effect)](/ru/essentials/work-with-async), выполняет свою конкретную роль.
<br/>
...
#### Особенности store (#store-features)

- У вас может быть столько store'ов, сколько вам нужно
...
// Обновляем store при срабатывании события
...
Рассмотрим пример из части про store, где мы имеем store с массивом суперлюдей. Допустим у нас появилось новое требование это выводить отдельно друг от друга героев и злодеев. Реализовать это будет очень просто при помощи производных store'ов:
...
В этом примере мы создали производные сторы `$superHeroes` и `$superVillains`, которые будут зависеть от оригинального `$supers`. При этом изменяя оригинальный store, у нас также будут изменяться и производные – это и есть реактивность!
...
Это рекомендации команды effector использовать `$` для store'ов и `fx` для эффектов.
