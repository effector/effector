...
Хранилище ([Store](/ru/api/effector/Store)) — это **центральный элемент управления состоянием** в Effector. Он представляет собой реактивное значение, которое может изменяться только через события, обеспечивая строгий контроль над мутациями и потоком данных.
