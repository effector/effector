...
Событие ([Event](/ru/api/effector/Event)) — для описания событий, как правило это взаимодейтсвие пользователя с вашей логикой:
...
- Они служат для уведомления других частей системы о произошедших изменениях.
- Можно создавать события, подписываться на их изменения и трансформировать их данные.
...
Хранилище ([Store](/ru/api/effector/Store)) — для управления состоянием вашего приложения
Особенности:
...
- Хранилище поддерживает реактивность — изменения автоматически распространяются на все подписанные компоненты.
  ...
  $counter.watch((state) => console.log(`Текущее значение: ${state}`));
  // Текущее значение: 0

  // Вызов события изменяет состояние
  increment(); // Текущее значение: 1
  ```

## Эффект (#effect)

Эффект ([Effect](/ru/api/effector/Effect)) — Эффект предназначен для обработки побочных действий в приложении. Это могут быть:

- Отправка HTTP-запросов.
- Чтение и запись данных в localStorage или indexDB.
- Взаимодействие с другими API или библиотеками.
...
fetchUserFx.fail.watch(() => console.log("Произошла ошибка!"));
...
$clickCount.on(buttonClicked, (state) => state + 1);
...
Более подробно о согласовании команды эффектор об именовании можно почитать [здесь](/ru/conventions/naming).
:::

Осталось связать изменение стора и вызов эффекта, и здесь в игру вступает прекрасный метод `sample`.
Более подробно о `sample` можно почитать [тут](/ru/api/effector/sample).
...
$clickCount.watch((state) => console.log(state));
