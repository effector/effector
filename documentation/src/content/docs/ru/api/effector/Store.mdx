...
`reducer` вызывается при каждом срабатывании `trigger`, а его возвращаемое значение становится новым состоянием стора. Как и при любом обновлении стора, возврат `undefined` пропускает обновление и оставляет состояние без изменений. Если стор не создан с опцией `skipVoid: false`, возврат `undefined` также выводит предупреждение, поэтому используйте его осознанно, чтобы пропустить обновление:

```ts
$store.on(trigger, (state, payload) => {
  if (!isValid(payload)) return; // пропустить обновление, оставить текущее состояние
  return next(state, payload);
});
```

Чтобы хранить `undefined` как настоящее значение, а не пропускать обновление, создайте стор с опцией `skipVoid: false`:
