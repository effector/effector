...
## What is Scope and why do we need it (#about-scope)

By default, all stores and events in Effector are static and global - they create one shared state for the entire application. But sometimes we need to isolate state.
Let's say we have a component that we want to display multiple times on a page, in our case it will be a basic counter:
...
// equivalent to creating an event using createEvent
// and subscribe the store to event via the on property
const { increment } = createApi($counter, {
  increment: (count) => count + 1,
});
```

Let's create a `Counter` component for demonstration:
...
Currently, when we use `$counter` directly without `Scope`, if we click the button and increase the value, the state will change for both components because all components work with the same global state directly:
...
However, this is not the behavior we expect.<br/>
We want to get isolated state, as in the case with `useState`, where each component has its own data, not shared. This is where the isolated context `scope` will help us:
...
`Scope` creates a separate copy of the state for each component instance. The original store remains unchanged!
:::
...
Meanwhile, our components look like this:
...
For other frameworks with integrations, it works identically.

Вы можете поиграть с этим примером [здесь](https://stackblitz.com/edit/effector-vite-react-template-ugavudbj?file=src%2Ftimer.tsx)
