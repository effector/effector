...
If you [work with scopes](/en/advanced/work-with-scope), this pattern may lead to [scope loss](/en/guides/scope-loss). In that case wrap the event with [`scopeBind`](/en/api/effector/scopeBind) in root of effect and use returned value as working event. No extra work is required outside of scoped environments.
