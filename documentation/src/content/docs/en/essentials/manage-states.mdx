...
State in effector is managed through stores - special objects that hold values and update them when receiving events. Stores are created using the [createStore](/api/effector/createStore) function.
...
2. Subscribe to changes via watch
...
3. Using framework integration hooks
