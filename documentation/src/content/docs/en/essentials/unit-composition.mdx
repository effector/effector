...
- You can update a [store](/en/api/effector/Store) by passing it to `target`, the old value will be fully replaced with the new one:
...
- You can also trigger an [effect](/en/api/effector/Effect) or [event](/en/api/effector/Event) by passing it to [`target`](/en/api/effector/sample#target-argument); the arguments will be forwarded declaratively to the `target`:
...
- **Articles**
  ...
  - [Description of common errors, gotchas and ways for solving them](/en/guides/troubleshooting)
