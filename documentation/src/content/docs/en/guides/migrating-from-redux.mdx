...
const fetchUserFx = createEffect((id) => api.getUser(id));
...
const $summary = combine($todos, $filter, (todos, filter) =>
...
const itemRequested = createEvent();
const fetchItemFx = createEffect((id) => api.getItem(id));

sample({
  clock: itemRequested,
  source: $cache,
  filter: (cache, id) => !cache[id],
  fn: (_, id) => id,
  target: fetchItemFx,
});
```
