...
description: Core concepets of effector - store, effect, event and how it works together
---
...
- Store data is immutable by default.
  ...
  $counter.watch((state) => console.log(`Current value: ${state}`));
...
$clickCount.watch((state) => console.log("Counter changed: ", state));
